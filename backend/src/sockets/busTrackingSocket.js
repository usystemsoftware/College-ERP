...
const registerBusTrackingHandlers = (socket) => {
  // Driver emits bus location
  socket.on('update_bus_location', (data) => {
    // Only authorized roles can emit bus location
    if (socket.user && !['Super Admin', 'College Admin', 'Transport Manager'].includes(socket.user.role)) {
      return socket.emit('bus_tracking_error', { message: 'Unauthorized to update bus location' });
    }
  ...
  socket.on('join_bus_overview', () => {
    // Only authorized roles can see the overview of all buses
    if (socket.user && !['Super Admin', 'College Admin', 'Transport Manager'].includes(socket.user.role)) {
      return socket.emit('bus_tracking_error', { message: 'Unauthorized to view bus overview' });
    }
